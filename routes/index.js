...
router.get("/", async function (req, res, next) {
  try {
    ...
    const url =
      ...
      encodeURIComponent("portfolio project in:readme user:rahulnshah");
...
router.get("/algorithms", (req, res) => {
  res.render("algorithms", {
    h1_text: "Core Data Structures and Algorithms",
  });
});
...
router.post("/api/email", function (req, res, next) {
  ...
  if (!result.valid) {
    let errors = result.errors.map((error) =>
      error.stack.substring(error.stack.lastIndexOf(".") + 1)
    );
